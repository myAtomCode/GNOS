...
/* DRM lease ioctls: this kernel has no lease support, but wlroots probes
 * CREATE_LEASE and needs the request recognised so it can fall back to a
 * plain open (ENOTTY would abort the probe). */
struct drm_mode_create_lease {
    __u64 object_ids;               /* __u64 *: array of object ids */
    __u32 object_count;
    __u32 flags;                    /* O_CLOEXEC */
    __u32 lessee_id;                /* out */
    __s32 fd;                       /* out */
};                                  /* 24 bytes */

struct drm_mode_list_lessees {
    __u32 count_flags;              /* bit 0 = only list lessees of master */
    __u32 pad;
    __u32 lessee_count;             /* out: number of lessees */
    __u32 pad2;
    __u64 lessee_ptr;               /* __u32 *: array of lessee ids */
};                                  /* 24 bytes */
...
struct drm_mode_get_lease {
    ...
    __u32 lessee_id;                /* out */
    __u32 pad2;
    __u64 fd_ptr;                   /* __u32 *: array of fds */
};                                  /* 24 bytes */
